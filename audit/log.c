...
static ih_core_bool_t write_entry_to_all_files(ih_audit_log_t *log,
...
ih_core_bool_t ih_audit_log_add_file(ih_audit_log_t *log, FILE *file)
{
  ...
  success = ih_container_set_add(log->files, file);
...
ih_audit_log_t *ih_audit_log_create(FILE *file)
{
  ...
  if (so_far_so_good) {
    log->files = ih_container_set_create(compare_files, IH_CORE_NO_COPY_FUNCTION,
        IH_CORE_NO_DESTROY_FUNCTION);
    if (log->files) {
      if (file) {
        if (ih_container_set_add(log->files, file)) {
          so_far_so_good = ih_core_bool_true;
        } else {
          so_far_so_good = ih_core_bool_false;
        }
  ...
  if (!so_far_so_good && log) {
    if (log->files) {
      ih_container_set_destroy(log->files);
    }
  }
    ...
    const char *entry, ...)
{
  ...
  if (ih_core_get_current_time_string(time_string)) {
    ...
    ih_core_trace("ih_core_get_current_time_string");
  }
...
ih_core_bool_t ih_audit_log_remove_file(ih_audit_log_t *log, FILE *file)
{
  ...
  pthread_mutex_lock(&log->mutex);
  success = ih_container_set_remove(log->files, file);
  pthread_mutex_unlock(&log->mutex);
...
ih_core_bool_t write_entry_to_all_files(ih_audit_log_t *log,
    char *formatted_entry)
{
  assert(log);
  assert(formatted_entry);
  ...
  pthread_mutex_lock(&log->mutex);
  {
    ih_container_set_iterate_start(log->files);
    while ((file = ih_container_set_iterate_next(log->files))) {

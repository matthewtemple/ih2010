    ...
    ih_audit_log_t *log)
{
  ...
  if (system) {
    ...
    ih_box_coordinate_init_witih_xyz(&dimension_coordinate,
        IH_BIOS_BOX_DIMENSION, IH_BIOS_BOX_DIMENSION, 1);
    ih_bios_actor_init_actorey(&system->actorey);
    ...
    ih_audit_log_trace(log, "bios", "malloc");
  }
    ...
    unsigned short max_solution_count)
{
  ...
  solutions = ih_core_create_solutions_from_box(system->box,
      max_solution_count, ih_bios_actor_get_solution, system->score_solution,
      system->goal, ih_bios_actor_compare_maximize,
      ih_bios_actor_compare_minimize, ih_bios_actor_copy, system->context,
      system->log);
  if (!solutions) {
    ih_audit_log_trace(system->log, "bios", "create_solutions_from_box");
  }
...
{
  ih_search_searchey_init(searchey, ih_bios_system_create,
      ih_bios_system_destroy, ih_bios_system_get_solutions_copy,
      ih_bios_system_search);
}

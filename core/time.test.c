#include "ih/core/constants.h"
#include "ih/core/days_hours_minutes_seconds.h"
#include "ih/core/time.h"
#include "ih/core/tools.h"
#include "ih/external/external.h"

int main(int argc, char *argv[])
{
  ih_core_time_t *time;
  ih_core_days_hours_minutes_seconds_t days_hours_minutes_seconds;
  unsigned long seconds;
  char *time_string;

  for (seconds = 1000000; seconds < 1000010; seconds++) {
    time = ih_core_time_create(seconds);
    if (time) {
      ih_core_time_get_days_hours_minutes_seconds(time,
          &days_hours_minutes_seconds);
      time_string = create_string_from_days_hours_minutes_seconds
        (&days_hours_minutes_seconds);
      if (time_string) {
        printf("%lu => %s\n", seconds, time_string);
        assert(seconds == days_hours_minutes_seconds.seconds
            + (days_hours_minutes_seconds.minutes * IH_CORE_SECONDS_PER_MINUTE)
            + (days_hours_minutes_seconds.hours * IH_CORE_SECONDS_PER_HOUR)
            + (days_hours_minutes_seconds.days * IH_CORE_SECONDS_PER_DAY));
        free(time_string);
      }
      ih_core_time_destroy(time);
    } else {
      ih_core_trace("ih_core_time_create");
    }
  }

  return 0;
}

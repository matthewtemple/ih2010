...
void ih_matih_stats_get_prf(unsigned long true_positives,
        ...
        double *f_measure)
{
    *precision = ih_matih_stats_get_precision(true_positives, false_positives);
    *recall = ih_matih_stats_get_recall(true_positives, false_negatives);
    *f_measure = ih_matih_stats_get_f_measure(*precision, *recall, recall_weight);
}

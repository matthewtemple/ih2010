...
static void ih_net_post_create_in_buffer(ih_net_post_t *post);

static ih_core_bool_t ih_net_post_create_inbox(ih_net_post_t *post);

static void ih_net_post_create_out_buffer(ih_net_post_t *post);

static ih_core_bool_t ih_net_post_create_outbox(ih_net_post_t *post);

static void ih_net_post_create_rollback(ih_net_post_t *post);

static void init_post_message_header(ih_net_post_t *post,
    post_message_header_t *post_message_header,
    ih_core_message_encoding_t encoding, ih_net_engine_id_t engine_id,
    unsigned long message_type, unsigned long data_size);

static void ntih_post_message_header
(post_message_header_t *post_message_header);

static ih_core_bool_t put_message_into_out_buffer(ih_net_post_t *post,
    ih_core_message_t *message);

static ih_core_bool_t put_received_message_in_inbox(ih_net_post_t *post);

static ih_core_bool_t receive_messages_body(ih_net_post_t *post);

static ih_core_bool_t receive_messages_header(ih_net_post_t *post);

static void reset_for_next_receive(ih_net_post_t *post);

static void reset_for_next_send(ih_net_post_t *post);

static void send_messages_get_new_message(ih_net_post_t *post);

static ih_core_bool_t send_messages_send_current_message(ih_net_post_t *post);

int ih_net_post_compare(void *post_object_a,
    void *post_object_b)
{
  int compare_result;
  ih_net_post_t *post_a;
  ih_net_post_t *post_b;
...
void *ih_net_post_create(int socket)
{
  ...
  if (post) {
    ...
    ih_net_post_create_in_buffer(post);
    ih_net_post_create_out_buffer(post);
  ...
  if (success) {
    success = ih_net_post_create_inbox(post);
  }

  if (success) {
    success = ih_net_post_create_outbox(post);
  }

  if (!success) {
    ih_net_post_create_rollback(post);
    post = NULL;
  }
...
void ih_net_post_create_in_buffer(ih_net_post_t *post)
{
  post->in_buffer = NULL;
...
ih_core_bool_t ih_net_post_create_inbox(ih_net_post_t *post)
{
  ih_core_bool_t success;

  post->inbox = ih_container_list_create(IH_CORE_NO_COMPARE_FUNCTION,
      ih_core_message_copy, IH_CORE_NO_DESTROY_FUNCTION);
  if (post->inbox) {
    success = ih_core_bool_true;
  } else {
...
void ih_net_post_create_out_buffer(ih_net_post_t *post)
{
...
ih_core_bool_t ih_net_post_create_outbox(ih_net_post_t *post)
{
  ih_core_bool_t success;

  post->outbox = ih_container_list_create(IH_CORE_NO_COMPARE_FUNCTION,
...
void ih_net_post_create_rollback(ih_net_post_t *post)
{
  if (post) {
    if (post->in_buffer) {
      free(post->in_buffer);
    ...
    if (post->inbox) {
      ih_container_list_destroy(post->inbox);
    }
    if (post->outbox) {
      ih_container_list_destroy(post->outbox);
    }
  }
}
...
void ih_net_post_destroy(void *post_object)
{
  ...
  ih_container_list_iterate_start(post->inbox);
  while ((message = ih_container_list_iterate_next(post->inbox))) {
...
void ih_net_post_get_stats(void *post_object,
...
void *ih_net_post_receive_message(void *post_object)
{
  ...
  message = ih_container_list_find_first(post->inbox);
  if (message) {
    ih_container_list_remove_first(post->inbox);
  }
...
ih_core_bool_t ih_net_post_send_message(void *post_object,
    void *message_object)
{
  assert(post_object);
  ...
  success = ih_container_list_add_last(post->outbox, message_object);
...
ih_container_list_t *ih_net_post_take_unsent_messages(ih_net_post_t *post)
{
  ...
  list = ih_container_list_create(IH_CORE_NO_COMPARE_FUNCTION,
      ih_core_message_copy, IH_CORE_NO_DESTROY_FUNCTION);
  if (list) {
    ih_container_list_iterate_start(post->outbox);
    while ((message = ih_container_list_iterate_next(post->outbox))) {
      if (ih_container_list_add_last(list, message)) {
        ih_container_list_iterate_remove(post->outbox);
      } else {
...
void init_post_message_header(ih_net_post_t *post,
    post_message_header_t *post_message_header,
    ih_core_message_encoding_t encoding, ih_net_engine_id_t engine_id,
    unsigned long message_type, unsigned long data_size)
{
...
void ntih_post_message_header(post_message_header_t *post_message_header)
{
...
ih_core_bool_t put_message_into_out_buffer(ih_net_post_t *post,
    ih_core_message_t *message)
{
  ih_core_bool_t success;
...
ih_core_bool_t put_received_message_in_inbox(ih_net_post_t *post)
{
  ih_core_bool_t success;
  ih_core_message_t *message;
  ...
  if (message) {
    if (ih_container_list_add_last(post->inbox, message)) {
      success = ih_core_bool_true;
    } else {
      ih_core_trace("ih_container_list_add_last");
      success = ih_core_bool_false;
    ...
    ih_core_trace("ih_core_message_create");
    success = ih_core_bool_false;
  }
...
ih_core_bool_t receive_messages_body(ih_net_post_t *post)
{
  assert(post);
...
ih_core_bool_t receive_messages_header(ih_net_post_t *post)
{
  ...
  if (actual_bytes_read > 0) {
    ...
    if ((sizeof post->in_message_header) == post->in_buffer_receive_position) {
      ntih_post_message_header(&post->in_message_header);
...
void reset_for_next_receive(ih_net_post_t *post)
{
  free(post->in_buffer);
  post->in_buffer = NULL;
  post->in_buffer_have_complete_header = ih_core_bool_false;
...
void reset_for_next_send(ih_net_post_t *post)
{
  free(post->out_buffer);
  post->out_buffer = NULL;
  post->currently_sending_out_buffer = ih_core_bool_false;
...
void send_messages_get_new_message(ih_net_post_t *post)
{
  ih_core_message_t *message;

  message = ih_container_list_find_first(post->outbox);
  if (message) {
    if (put_message_into_out_buffer(post, message)) {
      ih_container_list_remove_first(post->outbox);
...
ih_core_bool_t send_messages_send_current_message(ih_net_post_t *post)
{
